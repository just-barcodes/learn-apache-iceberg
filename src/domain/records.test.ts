...
describe("genRecords", () => {
  ...
  it("formats amount and date from the id", () => {
  ...
  it("emits only ids in the requested bucket", () => {
    const rows = genRecords(4, { month: "2026-02", bucket: 2 }, { oid: 1000 });
    expect(rows).toHaveLength(4);
    expect(rows.every((r) => r.order_id % 4 === 2)).toBe(true);
  });

  it("pins every row to a fixed day when requested", () => {
    const rows = genRecords(3, { month: "2026-03", day: 9 }, { oid: 2000 });
    expect(rows.every((r) => r.order_date === "2026-03-09")).toBe(true);
  });
});
